...
*/
SELECT m_name, instrument,
  COUNT(m_name) OVER (PARTITION BY instrument) total_players
...
SELECT temp1.m_name, COALESCE(comp_cnt, 0) compositions, COALESCE(instr_cnt, 0) instruments
...
(
  ...
  FROM musician m JOIN performer p ON m.m_no = p.perf_is
  JOIN plays_in pn ON p.perf_no = pn.player
  JOIN band b ON pn.band_id = b.band_no
  ...
  FROM musician m JOIN performer p ON m.m_no = p.perf_is
  JOIN plays_in pn ON p.perf_no = pn.player
  JOIN performance pf ON pn.band_id = pf.gave
...
SELECT m.m_name, pn.band_id, t.m_name, s.m_name
FROM musician m JOIN performer p ON m.m_no = p.perf_is
JOIN plays_in pn ON p.perf_no = pn.player
JOIN band b ON pn.band_id = b.band_no
JOIN place pl ON b.band_home = pl.place_no
LEFT JOIN temp_jd_bands t ON pn.band_id = t.band_id
LEFT JOIN temp_sl_bands s ON pn.band_id = s.band_id
WHERE (t.m_name IS NOT NULL) AND (s.m_name IS NOT NULL)
ORDER BY m.m_name, pn.band_id;
