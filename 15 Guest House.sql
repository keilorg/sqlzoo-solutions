...
SELECT departure, SUM(1st) 1st_floor, SUM(2nd) 2nd_floor, SUM(3rd) 3rd_floor
FROM temp3
GROUP BY departure;
...
*/
WITH temp1 AS (
  SELECT *,(booking_date + INTERVAL nights DAY) departure, @rownum:=@rownum + 1 rank1
  FROM booking, (SELECT @rownum:= 0) r1
  ORDER BY room_no, booking_date
), temp2 AS (
  SELECT (booking_date + INTERVAL nights DAY) previous_departure, @rownum2:=@rownum2 + 1 rank2
  FROM booking, (SELECT @rownum2:= 1) r2
  ORDER BY room_no, booking_date
), temp3 AS (
  SELECT *
...
*/
WITH temp1 AS (
  SELECT booking_id, SUM(amount) AS amt
  FROM extra
  GROUP BY booking_id
), temp2 AS (
