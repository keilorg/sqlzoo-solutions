...
WITH temp1 AS (
  SELECT
    taken_by,
    DATE_FORMAT(call_date, '%Y-%m-%d %H:%i:%s') last_call,
    @row_number1 := (CASE
    ...
    @first_call_date := (CASE
      ...
      END) AS first_call,
    @call_date := Issue.call_date call_date
  FROM Issue, (SELECT @row_number1 := 0, @call_date := 0, @first_call_date := 0) row_number_init
  ORDER BY taken_by, call_date
)
SELECT taken_by, first_call, last_call, call_count calls
FROM temp1
