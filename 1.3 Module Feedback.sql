...
WHERE (CAM_SMO.SPR_CODE = '50200100') AND
      (CAM_SMO.AYR_CODE = '2016/7') AND
      (CAM_SMO.PSL_CODE = 'TR1');
...
*/
SELECT INS_RES.QUE_CODE, QUE_TEXT,CAT_NAME, ROUND(100*SUM(FLOOR(RES_VALU/4))/COUNT(1)) percentage
FROM INS_RES JOIN INS_QUE ON INS_RES.QUE_CODE = INS_QUE.QUE_CODE
JOIN INS_CAT ON INS_QUE.CAT_CODE = INS_CAT.CAT_CODE
WHERE (INS_RES.MOD_CODE = 'SET08108') AND
      (INS_RES.AYR_CODE = '2016/7') AND
      (INS_RES.PSL_CODE = 'TR1')
...
*/
SELECT MOD_CODE,RES_VALU, COUNT(1) student_responses
FROM INS_RES
WHERE (INS_RES.MOD_CODE = 'SET08108') AND
      (INS_RES.AYR_CODE = '2016/7') AND
      (INS_RES.PSL_CODE = 'TR1') AND
      (INS_RES.QUE_CODE = '4.1')
