...
FROM world y
WHERE area = (SELECT MAX(area) FROM world x WHERE y.continent = x.continent);
...
SELECT name, continent
FROM world y
WHERE population = (SELECT MAX(population) FROM world x WHERE y.continent = x.continent) AND
      population > (SELECT 3*MAX(population) FROM world z WHERE y.continent = z.continent AND y.population != z.population)
